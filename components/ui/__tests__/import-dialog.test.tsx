...
function makeParseResult(toolCount: number = 2, errors: string[] = [], warnings: string[] = []) {
  const tools = Array.from({ length: toolCount }, (_, i) => ({
    ...
    icon: 'Terminal',
    parameters: [{ name: 'param1', type: 'string', description: 'Parameter 1', required: true }],
  }))
...
describe('ImportDialog', () => {
  ...
  describe('Visibility', () => {
    it('does not render when isOpen is false', () => {
      render(<ImportDialog isOpen={false} onClose={mockOnClose} onImport={mockOnImport} />)
      expect(screen.queryByText('Import from OpenAPI')).toBeNull()
    })

    it('renders when isOpen is true', () => {
      render(<ImportDialog isOpen={true} onClose={mockOnClose} onImport={mockOnImport} />)
