...
export function executeTool(tool: MCPTool, params: Record<string, unknown>): TestResult {
  // Validate parameters
...
export function testPrompt(prompt: MCPPrompt, args: Record<string, unknown>): TestResult {
  // Validate arguments
...
function generatePromptContent(prompt: MCPPrompt, args: Record<string, unknown>): string {
...
function validateSamplingConfig(sampling: SamplingConfig): ValidationError[] {
  ...
  if (sampling.enabled) {
    ...
    if (
      ...
      (sampling.temperature < 0 || sampling.temperature > 2)
    ) {
  ...
  if (elicitation.enabled) {
    ...
    if (elicitation.mode === 'form') {
      if (!elicitation.formFields || elicitation.formFields.length === 0) {
        errors.push({
          field: 'elicitation.formFields',
          message: 'At least one form field is required in form mode',
        })
      } else {
        for (let i = 0; i < elicitation.formFields.length; i++) {
          ...
          if (!field.name || field.name.trim() === '') {
            errors.push({
              field: `elicitation.formFields[${i}].name`,
  ...
  if (!resource.uri || resource.uri.trim() === '') {
    ...
    try {
      ...
      if (!resource.uri.includes('://')) {
        errors.push({
          field: 'uri',
          message: 'Resource URI must include a protocol (e.g., file://, db://)',
        })
      }
    } catch {
...
function matchTool(prompt: string, tools: MCPTool[]): MCPTool | null {
  ...
  for (const tool of tools) {
    ...
    for (const keyword of keywords) {
      if (
        toolKeywords.some((tk) => tk.includes(keyword)) ||
...
export function generateChatResponse(prompt: string, tools: MCPTool[]): ChatMessage {
  ...
  if (!simulation) {
    return {
      ...
      role: 'assistant',
      content:
        "I couldn't find a matching tool for that request. Try being more specific or add more tools to your server.",
      timestamp,
    }
  }
