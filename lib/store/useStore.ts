...
import type { ChatMessage, MCPPrompt, MCPResource, MCPServerConfig, MCPTool } from '../types'
  ...
  persist(
    (set, get) => ({
      ...
      updateTool: (id, updates) => {
        ...
        const updatedTools = tools.map((tool) => (tool.id === id ? updatedTool : tool))
        const updatedNodes = nodes.map((node) =>
          node.id === id ? { ...node, data: { ...node.data, tool: updatedTool } } : node
        )
      ...
      updateResource: (id, updates) => {
        ...
        const updatedNodes = nodes.map((node) =>
          node.id === id ? { ...node, data: { ...node.data, resource: updatedResource } } : node
        )
      ...
      updatePrompt: (id, updates) => {
        ...
        const updatedPrompts = prompts.map((prompt) => (prompt.id === id ? updatedPrompt : prompt))
        const updatedNodes = nodes.map((node) =>
          node.id === id ? { ...node, data: { ...node.data, prompt: updatedPrompt } } : node
        )
      ...
      pasteTool: () => {
        ...
        const newTool: MCPTool = {
          ...
          parameters: clipboard.parameters.map((p) => ({
            ...p,
            enum: p.enum ? [...p.enum] : undefined,
          })),
