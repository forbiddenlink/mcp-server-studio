import type { MCPParameter, MCPTool } from '../types'
...
function inferParameterType(
  ...
  // Number patterns
  if (
    /\b(id|count|limit|offset|page|size|amount|quantity|number|port|age|year|month|day)\b/.test(
      lowerName
    )
  ) {
    return 'number'
  ...
  // Array patterns
  if (
    /\b(list|array|items|tags|ids|values|options|filters|fields)\b/.test(lowerName) ||
...
function extractAction(
  description: string
): { action: string; category: keyof typeof actionPatterns } | null {
...
export function analyzeDescription(description: string): {
  ...
  if (params.length > 0) {
    ...
    suggestions.push(
      'Mention input parameters using "by", "with", or "given" (e.g., "by email", "with name and age").'
    )
  }
