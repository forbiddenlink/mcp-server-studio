...
import { analyzeDescription, parseToolDescription } from '../../generators/aiToolGenerator'
...
describe('aiToolGenerator', () => {
  describe('parseToolDescription', () => {
    ...
    it('infers boolean type for flag-like params', () => {
      ...
      const flagParam = tool.parameters.find(
        (p) => p.name.includes('is_active') || p.name.includes('flag')
      )
      if (flagParam) {
  ...
  describe('analyzeDescription', () => {
    ...
    it('caps confidence at 1.0', () => {
      const result = analyzeDescription('search users by email with name and age given an API key')
